...
describe("URL", async function() {
    ...
    describe("corbaloc", function() {
        describe("iiop", function() {
            it("with three hosts", function() {
                const parser = new UrlParser("corbaloc:iiop:1.1@mark-13.org:8080,iiop:1.2@mhsd.de:4040,:dawnrazor.co.uk/Prod/TradingService")
                expect(parser.parse()?.toString()).to.equal("corbaloc:iiop:1.1@mark-13.org:8080,iiop:1.2@mhsd.de:4040,iiop:1.0@dawnrazor.co.uk:2809/Prod/TradingService")
            })
            ...
            it("IPv6", function() {
                ...
                expect(parser.parse()?.toString()).to.equal("corbaloc:iiop:1.0@[1080::8:800:200C:417A]:2809/Prod/TradingService")
            })
        })
    ...
    describe("corbaname", function() {
        it("iiop", function() {
            ...
            expect(parser.parse()?.toString()).to.equal("corbaname:iiop:1.0@555objs.com:2809/NameService#a/string/path/to/obj")
        })
        it("rir", function() {
